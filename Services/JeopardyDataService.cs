...
using WthJeopardy.Models;

namespace WthJeopardy.Services;

public sealed class JeopardyDataService
{
    ...
    public async Task<JeopardyBoard> GetBoardAsync(CancellationToken cancellationToken = default)
    {
        ...
        var boardPath = Path.Combine(_environment.WebRootPath, "data", "jeopardy-board.json");
        ...
        var board = await JsonSerializer.DeserializeAsync<JeopardyBoard>(
            stream,
    ...
    private static JeopardyBoard CloneBoard(JeopardyBoard board)
    {
        return new JeopardyBoard
        {
            Categories = board.Categories
                .Select(category => new JeopardyCategory
                {
                    Name = category.Name,
                    Questions = category.Questions
                        .Select(question => new JeopardyQuestion
                        {
    ...
    private static JeopardyBoard BuildFallbackBoard()
    {
        return new JeopardyBoard
        {
            Categories = new List<JeopardyCategory>
            {
                new()
                {
                    ...
                    Questions = new List<JeopardyQuestion>
                    {
